    ...
    function (resp) { // Successfully get transfers
      ...
      for (var i = 0; i < l_tr; i++) {
        ...
        if (unique_transfers.indexOf(transfer_hash) == -1) {
          ...
          hash.id = 'tx-hash-' + k;
          hash.innerHTML = '<div class="in-tx-hash"><a target="_blank" href="http://coinevoblocks.info/search/' + transfer_hash + '">' + transfer_hash + '</a></div><div class="in-tx-hash-ell">...</div>';
...
function fillOutgoingTransactionTable(tx_table_id, transfer_type) {
  outgoingTxsDB.fetchOutgoingTxs( function (outgoingTxs) { // Successfully get transfers
    ...
    for (var i = 0; i < l_tr; i++) {
      ...
      pay_div.innerHTML = '<div class="out-pay-id-link"><span class="bold">Payment ID:</span> <a target="_blank" href="http://coinevoblocks.info/search/' + pay_id + '">' + pay_id + '</a></div><div class="in-tx-hash-ell">...</div>';
      ...
      for (var j = 0; j < hashes.length; j++) {
        hash_list.innerHTML += '<div class="outgoing-hash-link"><a target="_blank" href="http://coinevoblocks.info/search/' + hashes[i] + '">' + hashes[i] + '</a></div><div class="in-tx-hash-ell">...</div><br>';
      }
    ...
    function (resp) {
      ...
      if (resp.hasOwnProperty("result")) {
        ...
        for (var i=0; i < tx_hash_list.length; i++) {
          ...
          document.getElementById('send-txhashlist-popup').innerHTML += '<a target="_blank" href="http://explore.coinevoworld.com/search?value=' + this_hash + '">' + this_hash + '</a><br>';
        }
