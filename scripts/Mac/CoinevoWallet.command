...
echo "===================================================================================="
echo "                    Coinevo coinevo-wallet-cli Launcher for OS X"
echo ""
...
mkdir -p ~/Library/Application\ Support/CoinevoWalletChrome

sleep 1

cd ~/Library/Application\ Support/CoinevoWalletChrome
FILE="wallet.bin"
...
if [ -f "$FILE" ]; then
    ...
    if [ $DAEMON -eq 1 ]; then
        screen -S CoinevoWalletCLIrpc -d -m /Applications/CoinevoWallet/coinevo-wallet-cli --wallet-file $FILE --password $PASS --user-agent $USER --rpc-bind-ip 127.0.0.1 --rpc-bind-port 33332 --daemon-address mobile3.coinevo.tech:33330
    elif [ $DAEMON -eq 2 ]; then
        screen -S CoinevoWalletCLIrpc -d -m /Applications/CoinevoWallet/coinevo-wallet-cli --wallet-file $FILE --password $PASS --user-agent $USER --rpc-bind-ip 127.0.0.1 --rpc-bind-port 33332
    else
        screen -S CoinevoWalletCLIrpc -d -m /Applications/CoinevoWallet/coinevo-wallet-cli --wallet-file $FILE --password $PASS --user-agent $USER --rpc-bind-ip 127.0.0.1 --rpc-bind-port 33332 --daemon-address $CUSTOM_DAEMON
    fi
    ...
    if [ $DAEMON -eq 1 ]; then
        /Applications/CoinevoWallet/coinevo-wallet-cli --generate-new-wallet $FILE --password $NEWPASS --daemon-address mobile3.coinevo.tech:33330
    elif [ $DAEMON -eq 2 ]; then
        /Applications/CoinevoWallet/coinevo-wallet-cli --generate-new-wallet $FILE --password $NEWPASS
    else
        /Applications/CoinevoWallet/coinevo-wallet-cli --generate-new-wallet $FILE --password $NEWPASS --daemon-address $CUSTOM_DAEMON
    fi
